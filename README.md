...
### 🧅 5. Поддержка сети Tor (Hidden Services)
...
* **Локальный хостинг:** Сайт работает непосредственно на вашем устройстве, а трафик анонимно маршрутизируется через сеть Tor на локальный порт Flask (`8080` или `5000`).
