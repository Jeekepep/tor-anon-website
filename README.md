...
python app.py
```
После запуска проект будет доступен по адресу: `http://localhost:8080` (или `http://IP_АДРЕС_ТЕЛЕФОНА:8080` для локальной сети). Панель администратора находится по адресу `http://localhost:8080/admin`. Пароль по умолчанию  `prostest123`.
